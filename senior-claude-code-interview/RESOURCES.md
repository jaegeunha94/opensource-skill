...
- **(Day 6 조사, 2026-07-08) 신규 확인 문서**: `/en/security`(threat model, 내장 방어 계층, MCP/Cloud/IDE 보안, best practices), `/en/security-guidance`(security-guidance 플러그인 3단계 리뷰), `/en/agent-sdk/secure-deployment`(threat model 정의, Lethal Trifecta 링크, isolation 기술 비교, credential proxy 패턴), `/en/mcp`의 보안 관련 절(워크스페이스 신뢰와 `.mcp.json` 승인 게이팅, `requiresUserInteraction`)을 1차 출처로 확인.
- **(Day 6 조사) Threat model 공식 정의**: `/en/agent-sdk/secure-deployment`가 "이 도구들의 동작은 처리하는 콘텐츠(파일/웹페이지/사용자 입력)에 의해 영향을 받을 수 있다"고 명시하고, 이를 완화하는 원칙으로 isolation/least privilege/defense in depth 3가지를 제시한다. 같은 문서가 참고 링크로 Simon Willison의 "Lethal Trifecta"(민감 데이터 접근+비신뢰 콘텐츠 노출+외부 통신 능력)와 OWASP Top 10 for LLM Applications를 직접 인용한다.
- **(Day 6 조사) 내장 prompt injection 방어 목록**(`/en/security`): permission system, context-aware analysis, input sanitization, 네트워크 명령(curl/wget) 비자동승인, 웹검색 결과 요약(원문 대신 요약을 컨텍스트에 주입), WebFetch의 격리된 컨텍스트 윈도우, 신규 코드베이스/MCP 서버 trust verification(단 `-p` 비대화형 모드에서는 비활성화), 이미 허용목록에 있어도 의심스러운 조합이면 재승인 요구, fail-closed matching(매칭 안 되는 명령은 기본 수동 승인).
- **(Day 6 조사) Bash AST 파싱은 "permission gate, not a sandbox"**: `/en/agent-sdk/secure-deployment`가 명시적으로 "명령을 AST로 파싱해 권한 규칙에 매칭하지만, 명령의 대상 경로나 효과로부터 위험도를 추론하지 않는다"고 못박는다. `eval` 등 일부 구문은 allow 규칙과 무관하게 항상 승인 요구.
- **(Day 6 조사) MCP 워크스페이스 신뢰 게이팅(v2.1.196)**: `claude mcp list`/`claude mcp get`은 신뢰되지 않은 폴더에서 저장소에 커밋되지 않은 설정 파일의 승인만 읽는다. 프로젝트 `.claude/settings.json`에 커밋된 `enableAllProjectMcpServers`/`enabledMcpjsonServers`는 워크스페이스 신뢰 전까지 무시되고 서버는 `⏸ Pending approval` 상태로 멈춘다. 신뢰와 무관하게 적용되는 승인 출처: 사용자 `~/.claude/settings.json`, managed settings, `--settings`로 전달된 설정, git이 추적하지 않는 `.claude/settings.local.json`.
- **(Day 6 조사) `anthropic/requiresUserInteraction`(v2.1.199+)**: MCP 도구가 `tools/list` 응답에 이 메타데이터를 달면, `acceptEdits`/`auto`/`bypassPermissions`를 포함한 모든 모드에서 매번 프롬프트하고 "다시 묻지 않기" 옵션도 제공하지 않는다. `dontAsk` 모드에서는 승인 대신 거부된다. 비대화형 `--permission-prompt-tool`에서는 allow 응답이 자동으로 deny로 전환된다.
- **(Day 6 조사) 실제 공급망 사고 사례**: 보안 리서치(Check Point Research)가 보고한 CVE-2025-59536(프로젝트 훅 설정에 심어진 명령이 워크스페이스 신뢰 다이얼로그가 뜨기 전에 실행되는 코드 인젝션, 2025-10 v1.0.111에서 수정)과 CVE-2026-21852(환경 변수 override로 인증 트래픽을 공격자 인프라로 리다이렉트해 API 키 유출)를 사례로 확인. 별도로 npm 패키지의 postinstall hook이 `~/.claude.json`(MCP 라우팅 설정)을 재작성해 MCP 트래픽을 가로채는 공급망 공격 패턴이 CSO Online 등 여러 보안 매체에 보도됨. 이 항목들은 뉴스/벤더 보안 리서치 출처이며 이미 패치된 과거 사고이므로, 특정 버전·CVE 세부사항은 공식 보안 권고로 재확인하고 레슨에서는 "공급망을 통한 인젝션이 실제 사고로 이어진 패턴"이라는 일반화 수준까지만 근거로 사용한다.
- **(Day 6 조사) Credential proxy 패턴**(`/en/agent-sdk/secure-deployment`): `ANTHROPIC_BASE_URL`(Claude API 샘플링 요청만, 평문이라 프록시가 내용 검사/주입 가능) vs `HTTP_PROXY`/`HTTPS_PROXY`(시스템 전체, 그러나 HTTPS는 CONNECT 터널이라 프록시가 내용을 못 봄 — Node.js `fetch()`는 기본적으로 이 변수를 무시하며 Node 24+는 `NODE_USE_ENV_PROXY=1` 필요) vs TLS-terminating proxy(내용 검사/주입 가능하지만 agent 신뢰 저장소에 CA 인증서 설치 필요) 세 방식의 범위/한계가 다름. 격리 기술 비교표: Sandbox runtime(낮은 오버헤드, 커널 공유) < Containers < gVisor(유저스페이스 syscall 가로채기로 커널 공격 표면 축소, I/O 집약 워크로드는 최대 10~200배 느려질 수 있음) < VMs(Firecracker는 125ms 미만 부팅·5MiB 미만 오버헤드의 microVM).
- **(Day 6 조사) security-guidance 플러그인**(`/en/security-guidance`, 공식 마켓플레이스 플러그인, CLI v2.1.144+ 필요): 3단계 리뷰(파일 편집마다 무비용 패턴 매칭 → 턴 종료 시 별도 컨텍스트의 백그라운드 diff 리뷰 → 커밋/푸시 시 호출자·sanitizer까지 읽는 agentic 리뷰, 시간당 20회 상한) 전부 hooks(`SessionStart`/`UserPromptSubmit`/`PostToolUse`/`Stop`)로 구현됨. 리뷰 모델은 기본 Opus 4.7(`SECURITY_REVIEW_MODEL`/`SG_AGENTIC_MODEL`로 변경 가능). "같은 Claude 인스턴스가 자기 자신을 채점하지 않는다"는 독립성 설계이지만, 문서가 직접 "어떤 레이어도 쓰기/커밋을 막지 않으며 완전한 보안 솔루션이 아니라 defense-in-depth의 한 계층"이라고 명시. 계층형 스택: 세션 내(플러그인) → 온디맨드(`/security-review`) → PR(Code Review, Team/Enterprise) → CI(기존 정적 분석/의존성 스캐너).

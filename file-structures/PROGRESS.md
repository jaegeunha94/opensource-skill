...
| Day | 예정 주제 | 핵심 개념 |
|-----|-----------|-----------|
| 2 | 순차 파일 구조 | 순차 접근, 직렬 파일, 고정/가변 길이 레코드, 정렬 기준 |
| 3 | 인덱스 구조 기초 | 기본 인덱스, 보조 인덱스, 덴스/스파스 인덱스, 클러스터링 |
| 4 | 직접 파일 / 해싱 | 해시 함수, 충돌 해결(체이닝/오픈 어드레싱), 버킷, 적재율 |
| 5 | B-트리 | B-트리 구조, 차수(Order), 삽입/삭제 연산, 분할/합병 |
| 6 | B+-트리 | B+-트리 vs B-트리, 리프 노드 연결, 범위 검색 |
...
**Day 1 완료** — Day 2 (순차 파일 구조)를 시작할 준비가 됨.
...
- [ ] 순차 파일 상세 구조 (예정 Day 2)
- [ ] 인덱스 구조 — 덴스/스파스, 클러스터링 (예정 Day 3)
- [ ] 해싱 — 해시 함수, 충돌 해결 (예정 Day 4)
- [ ] B-트리 삽입/삭제 연산 (예정 Day 5)
- [ ] B+-트리 구조 및 범위 검색 (예정 Day 6)

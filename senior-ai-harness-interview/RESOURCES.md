...
## Handoff / Multi-Agent Coordination (Day 8)

- [How we built our multi-agent research system (Anthropic Engineering)](https://www.anthropic.com/engineering/multi-agent-research-system) — orchestrator-worker, lead(Opus)+3-5 subagent(Sonnet) 2단계 병렬화, artifact 패턴, 90.2% 성능 향상, 토큰 사용량 15x/성능 변동의 80% 설명
- [When to use multi-agent systems (and when not to) (Claude 블로그)](https://claude.com/blog/building-multi-agent-systems-when-and-how-to-use-them)
- [OpenAI Agents SDK — Handoffs](https://openai.github.io/openai-agents-python/handoffs/) — `transfer_to_<agent_name>` 도구화, `on_handoff`, `input_filter`
- [OpenAI Agents SDK — Orchestrating multiple agents](https://openai.github.io/openai-agents-python/multi_agent/) — handoff(제어권 이전) vs agents-as-tools(`agent.as_tool()`, 매니저 패턴) 비교
- [OpenAI Agents SDK — Multi-agent orchestration 가이드](https://developers.openai.com/api/docs/guides/agents/orchestration)
- [Claude Agent SDK — Subagents](https://code.claude.com/docs/en/agent-sdk/subagents) — 격리된 컨텍스트 윈도우, Task tool, 서브에이전트의 서브에이전트 스폰 금지(no nesting)
- [Cognition — Don't Build Multi-Agents](https://cognition.ai/blog/dont-build-multi-agents) — context sharing 부족·conflicting decision, "game of telephone"
- [Cognition — Multi-Agents: What's Actually Working](https://cognition.ai/blog/multi-agents-working) — map-reduce-and-manage(단일 쓰기 지점 + ephemeral child agent)로의 입장 수정, peer-to-peer GroupChat은 여전히 비권장
...
- Day 8의 `anthropic.com/engineering/multi-agent-research-system`, `openai.github.io/openai-agents-python/handoffs`·`multi_agent`, `cognition.ai/blog`(두 포스트 모두)는 403으로 원문 직접 접근이 막혀, 검색 스니펫과 다수의 독립 2차 출처(미러 블로그, SDK changelog, 커뮤니티 인용)로 교차검증했다. Anthropic의 90.2% 성능 향상·15x 토큰 비용·"토큰 사용량이 성능 변동의 80% 설명" 수치는 이 트랙 RESOURCES.md에 Day 1 작성 시점부터 이미 인용돼 있었고 이번에도 2개 이상 독립 출처에서 일치했지만, 정확한 인용이 필요하면 실제 브라우저로 원문을 재확인해야 한다. Cognition의 후속 포스트("Multi-Agents: What's Actually Working")는 정확한 발행일을 확인하지 못했으므로 "최근"이라고만 표현하고 특정 날짜를 단정하지 않는다. OpenAI Agents SDK의 `transfer_to_<agent_name>` 명명 규칙과 `input_filter`/`on_handoff` API, Claude Agent SDK의 subagent 격리 컨텍스트·Task tool·서브에이전트 nesting 금지는 각각 공식 문서 스니펫과 SDK changelog로 교차검증됐다.

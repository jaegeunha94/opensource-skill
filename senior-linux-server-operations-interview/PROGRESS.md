...
- [x] ulimit soft/hard limit과 systemd 서비스가 /etc/security/limits.conf를 건너뛰는 이유(PAM 세션 vs systemd fork) (Day 10)
- [x] sysctl 인터페이스와 /proc/sys, 즉시 적용(-w) vs 영속화(/etc/sysctl.d)의 차이 (Day 10)
- [x] TCP SYN 큐(tcp_max_syn_backlog)와 accept 큐(somaxconn)의 구분, syncookies trade-off (Day 10)

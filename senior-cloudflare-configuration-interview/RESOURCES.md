...
이 목록은 2026-07-04 기준으로 공식 문서/changelog/커뮤니티 공지를 확인해 작성했다(Day 2 작성 시 2026-07-05 기준으로 DNS/DNSSEC 관련 문서를, Day 3 작성 시 2026-07-06 기준으로 Cache Rules/Tiered Cache/캐시 스탬피드 관련 문서를, Day 4 작성 시 2026-07-07 기준으로 SSL/TLS 모드/AOP/Origin CA/mTLS 관련 문서를, Day 5 작성 시 2026-07-08 기준으로 WAF/Custom Rules/Rate Limiting Rules/Managed Rules 관련 문서를, Day 6 작성 시 2026-07-09 기준으로 DDoS Protection/Bot Management/Verified Bots/AI Crawl Control/Turnstile 관련 문서를 재확인해 링크를 추가함).
...
- **Rate Limiting Rules**
  ...
  — 구 Rate Limiting 상품도 2025-06-15 지원 종료, WAF 안의 Rate Limiting Rules로 통합. characteristic으로 IP, IP with NAT support(`_cfuvid` 쿠키), JA3/JA4 지원. mitigation timeout(10초~1일) 값 목록.
- **WAF phases / 보안 규칙 평가 순서**
  ...
  — `ddos_l7` → `http_request_firewall_custom`(Custom Rules) → `http_ratelimit`(Rate Limiting) → `http_request_firewall_managed`(Managed Rules) 순서. Custom Rules의 Skip으로 뒤 phase를 건너뛸 수 있는 이유.
- **Managed Rules troubleshooting / Overrides / Exceptions**
  ...
  — Overrides(규칙/태그 단위 액션·민감도 조정)와 Exceptions(조건부 평가 제외, Overrides보다 우선순위 높음) 구분. 7일 릴리스 사이클(1주차 Log → 2주차 기본 액션, 긴급 CVE는 예외).
- **OWASP Core Ruleset**
  https://developers.cloudflare.com/waf/managed-rules/reference/owasp-core-ruleset/
  — Cloudflare Managed Ruleset + WAF Attack Score 위에 추가 시 오탐이 잦고 이득이 제한적이라고 공식 명시. 폐기되지는 않았으나 보조적 위치.
- **DDoS Protection 개요 / 공격 범위 / Adaptive Protection**
  ...
  — 상시 자동 L3/4(네트워크) + L7(애플리케이션) 방어, Adaptive DDoS Protection(Enterprise).
- **HTTP / Network-layer DDoS Attack Protection — override / sensitivity**
  ...
  https://developers.cloudflare.com/ddos-protection/managed-rulesets/network/
  ...
  — 기본 민감도 High, Enterprise Advanced DDoS Protection 애드온에서 scope 지정 override 최대 10개.
- **DDoS Threat Report 2025 Q1/Q2/Q4 (공식 블로그)**
  ...
  — 2025 Q4 31.4 Tbps(35초) 신기록, Aisuru-Kimwolf 봇넷의 200M+ rps HTTP DDoS, 모두 상시 자동 방어로 처리.
- **Bot Fight Mode / Super Bot Fight Mode / Bot Management**
  ...
  — `cf.bot_management.score`(1~99)는 Enterprise Bot Management 전용, Super Bot Fight Mode는 4-카테고리 분류(verified bot/likely automated/definitely automated/likely human)만 제공. Verified Bots는 역방향 DNS/IP 대역 검증 등 스푸핑 방지 요건 동반.
- **AI Crawl Control**
  ...
  — AI 크롤러별 allow/charge(pay-per-crawl, 미결제 시 402)/block 정책, robots.txt Content Signals(search/ai-input/ai-train).
- **Turnstile / Cloudflare Challenges**
  https://developers.cloudflare.com/turnstile/
  https://developers.cloudflare.com/turnstile/changelog/
  ...
  — 위젯 모드(Managed/Non-Interactive/Invisible), Managed Challenge가 WAF 기본 권장 액션, 2025 하반기부터 네트워크 레벨 시그널 비중 강화, 2026-06 Turnstile Events Logpush 데이터셋 추가.

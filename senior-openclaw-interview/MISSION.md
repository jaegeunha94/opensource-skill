...
- **모델 장애 대응은 2단계 구조**다 — 같은 provider 안에서의 auth profile rotation(자격 증명 교체) 후, 그래도 실패하면 `agents.defaults.model.fallbacks`에 정의된 다음 모델로 전환한다. 프로필은 세션당 pin되어 provider 캐시를 따뜻하게 유지한다.
